#!/usr/bin/env python3
"""Update Beauty Lashes to bright light theme"""
import os

BASE = '/var/www/lashes.3nd.dev'
LOGO_WRAPPER = '''<div style="background:#fff;padding:6px 10px;border-radius:10px;border:2px solid var(--border);display:inline-block;margin:-6px 0"><img src="/images/logo.jpg" alt="Beauty Lashes" style="height:50px;border-radius:6px"></div>'''

CSS = r'''/* =============================================
   BEAUTY LASHES — Bright Betty Boop Era
   ============================================= */
...
nav {
  ...
  background: rgba(253,248,244,0.95); backdrop-filter: blur(16px);
...
.nav-links a { font-size: 13px; font-weight: 500; letter-spacing: 0.8px; text-transform: uppercase; color: var(--text-warm); position: relative; }
.nav-links a::after { content: ''; position: absolute; bottom: -4px; left: 0; right: 0; height: 2px; background: var(--pink); transform: scaleX(0); transition: transform 0.25s ease; }
...
.hero {
  ...
  background: var(--cream); padding: 100px 24px 60px;
}
.hero-sparkles {
  position: absolute; inset: 0; pointer-events: none;
  background:
    radial-gradient(circle at 10% 20%, rgba(255,45,111,0.08) 0%, transparent 40%),
    radial-gradient(circle at 90% 80%, rgba(255,45,111,0.06) 0%, transparent 40%),
    radial-gradient(circle at 50% 50%, rgba(201,146,42,0.04) 0%, transparent 60%);
}
.hero-content { position: relative; z-index: 2; text-align: center; max-width: 780px; }
.hero-logo-wrap { background: #fff; border-radius: 20px; padding: 24px; display: inline-block; margin-bottom: 24px; box-shadow: 0 4px 24px rgba(0,0,0,0.1); border: 2px solid var(--border); }
.hero-logo-wrap img { height: 180px; border-radius: 12px; }
...
.hero h1 { font-size: clamp(48px, 8vw, 88px); color: var(--black); margin-bottom: 20px; font-style: italic; animation: fadeIn 0.8s ease 0.3s both; }
.hero h1 span { color: var(--pink-deep); }
.hero-sub { font-family: 'Cormorant Garamond', serif; font-size: clamp(16px, 2vw, 22px); font-style: italic; color: var(--text-warm); margin-bottom: 40px; animation: fadeIn 0.8s ease 0.4s both; }
.hero-actions { display: flex; gap: 16px; justify-content: center; flex-wrap: wrap; animation: fadeIn 0.8s ease 0.5s both; }
.hero-scroll { position: absolute; bottom: 32px; left: 50%; transform: translateX(-50%); z-index: 2; display: flex; flex-direction: column; align-items: center; gap: 8px; color: var(--text-muted); font-size: 10px; letter-spacing: 2px; text-transform: uppercase; animation: bounce 2s infinite; }
...
.service-book { width: 100%; justify-content: center; padding: 12px; background: var(--cream-warm); color: var(--pink-deep); border: 2px solid var(--pink-light); font-size: 12px; }
...
/* PAGE HEADERS */
.page-header { padding: 130px 0 60px; background: var(--cream-warm); text-align: center; position: relative; }
.page-header::before { content: ''; position: absolute; inset: 0; background: radial-gradient(ellipse at 50% 0%, rgba(255,45,111,0.06), transparent 60%); }
.page-header h1 { font-size: clamp(28px, 5vw, 52px); margin-bottom: 10px; position: relative; }
.page-header p { color: var(--text-warm); font-family: 'Cormorant Garamond', serif; font-size: 18px; font-style: italic; position: relative; }
...
@media(max-width: 768px) {
  .nav-links { display: none; flex-direction: column; position: absolute; top: 72px; left: 0; right: 0; background: rgba(253,248,244,0.98); border-bottom: 2px solid var(--border); padding: 20px 24px; gap: 12px; }
...
# Logo wrapper HTML (white bg for dark logo)
LOGO_NAV = '<div style="background:#fff;padding:6px 10px;border-radius:10px;border:2px solid var(--border);display:inline-block;margin:-6px 0"><img src="/images/logo.jpg" alt="Beauty Lashes" style="height:50px;border-radius:6px"></div>'
LOGO_HERO = '<div style="background:#fff;border-radius:20px;padding:24px;display:inline-block;margin-bottom:24px;box-shadow:0 4px 24px rgba(0,0,0,0.1);border:2px solid var(--border)"><img src="/images/logo.jpg" alt="Beauty Lashes" style="height:180px;border-radius:12px"></div>'
...
print('\nAll files updated!')
print(f'CSS + all templates written to {BASE}')