...
try {
  ...
  // Load settings from DB into business object
  const rows = db.prepare("SELECT * FROM settings").all();
  rows.forEach(r => { siteData.business[r.key] = r.value; });
} catch(e) { console.log("Site data load:", e.message); }
...
app.post("/booking", (req, res) => {
  ...
  if (!name || !phone || !email || !service) return res.json({ error: "Please fill in all required fields." });
...
app.post("/admin/gallery/upload", require("express").json(), (req, res, next) => {
...
app.post("/admin/gallery/upload", upload.single("image"), (req, res) => {
  ...
  db.prepare("INSERT INTO gallery (caption, image, category) VALUES (?,?,?)").run(caption, req.file.filename, category);
  res.redirect("/admin/gallery");
});
